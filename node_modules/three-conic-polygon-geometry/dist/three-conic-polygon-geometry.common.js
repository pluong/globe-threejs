...
function geoPolygonTriangulate(polygon) {
  ...
  // calc uvs
  var lngUvScale = d3Scale.scaleLinear(d3Array.extent(points, function (d) {
    return d[0];
  }), [0, 1]);
  var latUvScale = d3Scale.scaleLinear(d3Array.extent(points, function (d) {
    return d[1];
  }), [0, 1]);
...
function pointInside(pnt, polygon) {
  ...
  return crossesPoleOrAntimeridian ? d3Geo.geoContains(polygon, pnt) : turfPointInPolygon(pnt, polygon);
}
...
var ConicPolygonBufferGeometry = /*#__PURE__*/function (_THREE$BufferGeometry) {
  ...
  function ConicPolygonBufferGeometry(polygonGeoJson, startHeight, endHeight, closedBottom, closedTop, includeSides, curvatureResolution) {
    ...
    var flatUvs = d3Array.merge(triangles.uvs);
    var vertices = [];
    var uvs = [];
    var indices = [];
