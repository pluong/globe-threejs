...
var _bfg__namespace = /*#__PURE__*/_interopNamespaceDefault(_bfg);
  ...
  update: function update(state) {
    ...
    var byH3Idx = indexBy(state.hexBinPointsData.map(function (d) {
      return _objectSpread2(_objectSpread2({}, d), {}, {
        h3Idx: h3Js.latLngToCell(latAccessor(d), lngAccessor(d), state.hexBinResolution)
      });
    }), 'h3Idx');
    ...
    function createObj(d) {
      ...
      obj.__hexCenter = h3Js.cellToLatLng(d.h3Idx);
      obj.__hexGeoJson = h3Js.cellToBoundary(d.h3Idx, true).reverse(); // correct polygon winding
    ...
    function updateObj(obj, d) {
      ...
      obj.geometry && obj.geometry.dispose();
      obj.geometry = new threeConicPolygonGeometry.ConicPolygonGeometry([geoJson], 0, GLOBE_RADIUS, false, true, true, topCurvatureResolution);
      var targetD = {
        alt: +altitudeAccessor(d)
      };
      var applyUpdate = function applyUpdate(td) {
    ...
    threeDigest(singlePolygons, state.scene, {
      ...
      updateObj: function updateObj(obj, _ref) {
        ...
        if (!objMatch(conicObj.geometry.parameters || {}, {
          ...
          conicObj.geometry = new threeConicPolygonGeometry.ConicPolygonGeometry(coords, 0, GLOBE_RADIUS, false, true, true, capCurvatureResolution);
        }
        if (addStroke && (!strokeObj.geometry.parameters || strokeObj.geometry.parameters.geoJson.coordinates !== coords || strokeObj.geometry.parameters.resolution !== capCurvatureResolution)) {
          ...
          strokeObj.geometry = new threeGeojsonGeometry.GeoJsonGeometry({
            type: 'Polygon',
            coordinates: coords
    ...
    threeDigest(state.hexPolygonsData, state.scene, {
      ...
      updateObj: function updateObj(obj, d) {
        ...
        if (Object.keys(targetD).some(function (k) {
          ...
          var h3Idxs = [];
          if (geoJson.type === 'Polygon') {
            h3Js.polygonToCells(geoJson.coordinates, h3Res, true).forEach(function (idx) {
              return h3Idxs.push(idx);
            });
          } else if (geoJson.type === 'MultiPolygon') {
            geoJson.coordinates.forEach(function (coords) {
              return h3Js.polygonToCells(coords, h3Res, true).forEach(function (idx) {
                return h3Idxs.push(idx);
              });
            });
          } else {
          ...
          var hexBins = h3Idxs.map(function (h3Idx) {
            var hexCenter = h3Js.cellToLatLng(h3Idx);
            var hexGeoJson = h3Js.cellToBoundary(h3Idx, true).reverse(); // correct polygon winding
          ...
          var applyUpdate = function applyUpdate(td) {
            ...
            obj.geometry = !hexBins.length ? new THREE$9.BufferGeometry() : BufferGeometryUtils.mergeBufferGeometries(hexBins.map(function (h) {
              ...
              return new threeConicPolygonGeometry.ConicPolygonGeometry([geoJson], GLOBE_RADIUS, GLOBE_RADIUS * (1 + alt), false, true, false, curvatureResolution);
            }));
          };
